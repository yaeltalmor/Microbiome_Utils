...
from temporal_transformer_wo_time import TemporalTransformerClassifier, EventEmbAggregateMethod, \
...
def train_eval(model, train_data, val_data, lr, max_epochs=50, batch_size=32,
                weight_decay=1e-5, log_dir="./runs/exp1",
    ...
    # TODO: old version: optimizer reduces LR when validation stops improving, giving the model a chance to refine.
    # scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='max', factor=0.5, patience=3, verbose=True)
    # If you see the curves jump early and then plateau, try halving the LR (e.g. 1e-3 → 5e-4 or 1e-4 → 5e-5).
    # Epoch-based LR schedule (does NOT depend on validation metrics)
    ...
    for epoch in range(max_epochs):
        ...
        # scheduler.step(val_ap) # old version
        scheduler.step()
        current_lr = optimizer.param_groups[0]["lr"]

        # TensorBoard logging
        if tb_writer:
            tb_writer.add_scalar("Loss/train_epoch", train_loss_eval, epoch)
    ...
    return val_auc, val_ap, best_epoch, fpr, tpr, prec, rec
...
def run_cv(model_params, idx, labels_np, config_name, save_best, run_tag,
           ...
           max_epochs, n_splits, repeat_idx):
    ...
    for fold, (train_fold_idx, val_fold_idx) in enumerate(
        ...
        auc, ap, best_epoch, fpr, tpr, prec, rec = train_eval(
            ...
            weight_decay=model_params["capacity"]["weight_decay"],
            log_dir=f"./runs_{GRID_BATCH}/{config_name}{run_tag}_fold{fold}",
...
def grid_param_check(param_grid,
                     train_idx, labels_np,
                     event_idx, value_idx, numeric_value, value_type_mask,
                     t_values, src_mask, labels,
                     event_type_vocab, cat_value_vocab,
                     n_splits=5, n_repeats=10, max_epochs=50, results_dir="./results"):
    results_dict = {}
    ...
    for config_name, params in param_grid.items():
        ...
        res = run_cv(params, train_idx, labels_np, config_name,
                     ...
                     max_epochs=max_epochs, n_splits=n_splits, repeat_idx=0)
        ...
        # (B) Seeded cross-validation
        for repeat_idx in range(1,n_repeats+1):
            # boot_sample = train_idx[torch.randint(
            #     low=0, high=len(train_idx), size=(len(train_idx),), device=train_idx.device
            # )]

            res_b = run_cv(params, train_idx, labels_np, config_name,
                           save_best=False, run_tag=f"_repeat_{repeat_idx}",
                           event_idx=event_idx, value_idx=value_idx,
                           numeric_value=numeric_value, value_type_mask=value_type_mask,
                           t_values=t_values, src_mask=src_mask,
                           metadata_idx=params["metadata_size_and_data"][1], labels=labels,
                           event_type_vocab=event_type_vocab, cat_value_vocab=cat_value_vocab,
                           max_epochs=max_epochs, n_splits=n_splits, repeat_idx=repeat_idx)
        ...
        aupr_diff = mean_ap - pos_rate
        aupr_diff_to_neg = aupr_diff / (1 - pos_rate)
        ...
        results_dict[config_name] = {
            "params": f"{params}",
            "AP_diff_to_neg": aupr_diff_to_neg,
            "AP_SNR": ap_snr,
            "AP_mean": mean_ap,
            "AP_std": std_ap,
            "AUC_SNR": auc_snr,
        ...
        print(f"{config_name}: "
              f"AP_diff_to_neg={aupr_diff_to_neg:.3f}, "
              f"AP={mean_ap:.3f} +- {std_ap:.3f}, "
              f"AUC={mean_auc:.3f} +- {std_auc:.3f}, "
              f"Epoch={mean_epochs:.1f} +- {std_epochs:.3f},")

    # Pick best by AP_diff_to_neg instead of AP_mean
    sorted_cfgs = sorted(results_dict.items(), key=lambda x: x[1]["AP_diff_to_neg"], reverse=True)

    print("\nTop 3 configs by AP_diff_to_neg:")
    for cfg, stats in sorted_cfgs[:3]:
        print(f"{cfg} → AP_diff_to_neg={stats['AP_diff_to_neg']:.3f}, "
...
def run_model_selection(param_grid_combination, train_idx, transformer_batch: TransformerBatch = None, NUM_YEARS: int = 2):
    labels_np = transformer_batch.labels.cpu().numpy()
    ...
    results_dict, best_cfgs = grid_param_check(
        param_grid=param_grid_combination,
        train_idx=train_idx, labels_np=labels_np,
        event_idx=transformer_batch.event_idx, value_idx=transformer_batch.value_idx,
        numeric_value=transformer_batch.numeric_value, value_type_mask=transformer_batch.value_type_mask,
        t_values=transformer_batch.t_values, src_mask=transformer_batch.src_mask,
        labels=transformer_batch.labels,
        event_type_vocab=transformer_batch.event_type_vocab, cat_value_vocab=transformer_batch.cat_value_vocab,
        n_splits=5, n_repeats=10, max_epochs=50,
        results_dir=f"./results_{GRID_BATCH}"
    )

    return results_dict, best_cfgs

def get_finalists_grid(transformer_batch: TransformerBatch = None):
    # === Base shared settings ===
    base_capacity = [
        {"lr": 1e-4, "dropout": 0.3,
         "num_heads": 1, "d_ff": 128, "num_layers": 1,
         "cls_hidden": 64, "weight_decay": 1e-5},
        {"lr": 1e-4, "dropout": 0.3,
         "num_heads": 1, "d_ff": 256, "num_layers": 2,
         "cls_hidden": 64, "weight_decay": 1e-5},
    ]

    # === Fixed order of parameters (same as original tournament) ===
    def make_param_grid(transformer_batch, initial_emb_dim, time_emb_type):
        return dict(
            capacity=base_capacity,
            pooling=["max", "mean"],
            initial_emb_dim=[initial_emb_dim],
            event_emb_agg_method=[EventEmbAggregateMethod.SUM],
            event_time_emb_agg_method=[
                EventTimeEmbAggregateMethod.SUM,
                EventTimeEmbAggregateMethod.CONCAT
            ],
            time_emb_type=[time_emb_type],
            md_token=[True, False],
            metadata_size_and_data=[
                (5, transformer_batch.metadata_idx, "metadata_idx"),
                (7, transformer_batch.metadata_weight_idx, "metadata_weight_idx"),
            ],
        )

    # === Define partial grids (keeping exact key order) ===
    partial_grids = {
        "transformer_training_utils_CVE_32": make_param_grid(transformer_batch, 32, TimeEmbeddingType.CVE),
        "transformer_training_utils_CVE_64": make_param_grid(transformer_batch, 64, TimeEmbeddingType.CVE),
        "transformer_training_utils_TIME2VEC_32": make_param_grid(transformer_batch, 32, TimeEmbeddingType.TIME2VEC),
        "transformer_training_utils_TIME2VEC_64": make_param_grid(transformer_batch, 64, TimeEmbeddingType.TIME2VEC),
        "transformer_training_utils_REL_POS_32": make_param_grid(transformer_batch, 32, TimeEmbeddingType.REL_POS_ENC),
        "transformer_training_utils_REL_POS_64": make_param_grid(transformer_batch, 64, TimeEmbeddingType.REL_POS_ENC),
    }
    # === Top combos as before ===
    top_combos_per_run = {
        "transformer_training_utils_CVE_32": ["combo_17", "combo_15", "combo_22"],
        "transformer_training_utils_CVE_64": ["combo_26", "combo_15", "combo_28"],
        "transformer_training_utils_TIME2VEC_32": ["combo_27", "combo_12", "combo_13"],
        "transformer_training_utils_TIME2VEC_64": ["combo_31", "combo_8", "combo_27"],
        "transformer_training_utils_REL_POS_32": ["combo_30", "combo_28", "combo_14"],
        "transformer_training_utils_REL_POS_64": ["combo_27", "combo_13", "combo_24"],
    }
    final_top_param_list = []
    for run_name, combo_ids in top_combos_per_run.items():
        grid = partial_grids[run_name]

        # re-enumerate this run’s combinations (as was done in each subrun)
        keys, values = zip(*grid.items())
        combos = [dict(zip(keys, v)) for v in itertools.product(*values)]

        combo_dict = {f"combo_{i}": combo for i, combo in enumerate(combos)}

        # extract top 3 configs for this run
        for cid in combo_ids:
            if cid not in combo_dict:
                print(f"⚠️ Warning: {cid} not found in {run_name} (check enumeration count)")
                continue
            final_top_param_list.append(combo_dict[cid])
    param_grid_combination = {
        f"combo_{i}": combo for i, combo in enumerate(final_top_param_list)
    }
    return param_grid_combination

def main():
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    transformer_batch=load_data(NUM_YEARS=4, device=device)
    # train_idx, test_idx = train_test_outer_split(transformer_batch, NUM_YEARS=4) # TODO: maybe delete this
    train_idx = torch.arange(transformer_batch.event_idx.size(0))
    param_grid_combination = get_finalists_grid(transformer_batch)
    results_dict, best_cfgs = run_model_selection(param_grid_combination, train_idx, transformer_batch, NUM_YEARS=4)
